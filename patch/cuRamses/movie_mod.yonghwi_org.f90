...
subroutine output_frame()
  ...
  real(dp)::xtmp,ytmp,ztmp,smooth,dist_cam,theta_cam,phi_cam,alpha,beta,smooth_theta
 ...
 do proj_ind=1,LEN(trim(proj_axis)) 
...
#ifdef NOSYSTEM
     if(myid==1)call PXFMKDIR(TRIM(moviedir),LEN(TRIM(moviedir)),O'775',info)  
#else
  ...
  dist_cam   = boxlen
  if((focal_camera(proj_ind).le.0D0).or.(focal_camera(proj_ind).gt.dist_cam)) focal_camera(proj_ind) = dist_cam
#if NDIM>2                 
