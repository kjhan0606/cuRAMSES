...
subroutine splitworkbroadcast(nwork,niwork,nwwork,input,recvcnt,displs)
   use amr_commons
   implicit none
   include 'mpif.h'
   integer:: nwork,nwwork,i,ierr,niwork
   integer, dimension(1:nwork):: recvcnt, displs
   real(dp), dimension(1:nwork):: input
   real(dp), allocatable, dimension(:):: work
   allocate(work(1:nwork))
   call MPI_Gatherv(input(niwork),nwwork, MPI_DOUBLE_PRECISION, work, recvcnt,displs, &
                &   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
   call MPI_Bcast(work, nwork, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
   do i = 1, nwork
      input(i) = work(i)
   enddo
   deallocate(work)
...
end subroutine splitworkbroadcast



subroutine mpiworksplit(nid,myid,nsink,nisink,njsink,nwsink, displs,recvcnt)
  implicit none
  integer:: nid,myid,nsink,nisink,njsink,nwsink,i
  integer:: displs(nid), recvcnt(nid)
  ...
  return
end subroutine mpiworksplit




function Division(thisnode, pnum)
  use Tree_commons
  implicit none
  logical Division
...
subroutine EraseFromTree(node, particle,np, optr, ptr, nptr)
  ...
  implicit none
  integer:: np
  TYPE(Tree_Node), dimension(1:2*np):: node
...
function pfof_open(node, particle,np, inow, ptr, foflink, boxsize) 
  use Tree_commons
  implicit none
...
!$omp parallel  private(mythr, nthr, istep, istart, ifinal, iflag,ii,gindx,gnum,iflag,j,icount)
...
recursive function Make_Tree(node,particle,np,iworknode, iSpareNode0) result(iNextSpareNode)
  ...
  implicit none
  integer:: np
  TYPE(Tree_Node),  target, dimension(1:2*np):: node
