...
!##############################################
!##############################################
subroutine sub_rho_from_current_level(ilevel, igrid, nump)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
...
subroutine cic_amr(ind_cell,ind_part,ind_grid_part,x0,ng,np,ilevel)
  ...
  do ind=1,twotondim
    ...
    if(sink)then
     ...
     ! For low mass baryon particles (could be stars or BH clouds)
     if(star)then
    if(sink)then
       do j=1,np
...
!$omp parallel do private(igrid,ngrid)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     call sub_multipole_fine(ilevel,igrid,ngrid,xc)
  enddo
  ! Update boundaries
  do idim=1,ndim+1
     call make_virtual_fine_dp(unew(1,idim),ilevel)
...
subroutine sub_multipole_fine(ilevel, igrid,ngrid,xc)
  use amr_commons
  use hydro_commons
  use poisson_commons
  implicit none
...
subroutine cic_from_multipole(ilevel)
  ...
  integer ::idim,nleaf,ix,iy,iz,igrid,ierror
#ifndef _OPENMP
...
!!$omp parallel do private(igrid,ngrid)
     do igrid=1,ncache,nvector
        ...
        ngrid=MIN(nvector,ncache-igrid+1)
        call cic_cell(ilevel,igrid,ngrid)
     end do
  end if
...
!###########################################################
!###########################################################
subroutine cic_cell(ilevel,jgrid,ngrid)
  use amr_commons
  use poisson_commons
...
subroutine tsc_amr(ind_cell,ind_part,ind_grid_part,x0,ng,np,ilevel)
  ...
  real(dp),dimension(1:nvector,1:threetondim)::vol
  integer ,dimension(1:nvector,1:threetondim)::igrid,icell,indp,kg
  real(dp),dimension(1:3)::skip_loc
