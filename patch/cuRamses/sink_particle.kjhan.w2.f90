...
!################################################################
!################################################################
subroutine kjhan_make_sink(ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  ...
  real(dp)::velc,uc,vc,wc,l_jeans,d_jeans,d_thres,dd_sink
  ...
  integer ,pointer:: ind_grid(:), ind_cell(:)
  logical ,pointer:: ok(:)
  integer mythread, nthreads,nwork,icount,jcount
  common /omp1_threads/ mythread
!$omp threadprivate(/omp1_threads/)
  integer ioffsetthread
  integer, dimension(:), allocatable:: iokthread

#ifndef _OPENMP
  integer ,dimension(1:nvector)::ind_grid_new,ind_cell_new,ind_part
#else
  integer ,dimension(:), allocatable::ind_grid_new,ind_cell_new,ind_part,ind_ind
  integer ,dimension(:), allocatable::ind_part_cloud,ind_grid_cloud, ok_true
#endif


  common /openmpmk_sink/ ind_grid, ind_cell, ok,  ioffsetthread
!$omp threadprivate(/openmpmk_sink/)
...
!$omp end parallel
  ...
  allocate(Pok(1:nvector,0:nthreads-1))
  allocate(iokthread(0:nthreads-1))
  iokthread = 0
!$omp parallel
...
!$omp parallel
     ...
     ioffsetthread = sum(iokthread(0:mythread-1))
     endif
!$omp end parallel
#endif
  ! Loop over grids
   ...
   ! Gather cells with a new sink
        do i=1,ngrid
           if (flag2(ind_cell(i))>0)then
              ...
              point2flag2(ninc)=ind_cell(i) 
              ! This is a local pointer that is not shared with other cpus
#else
              nnew=izero_myid+ninc+ioffsetthread
              x_tmp(nnew,1)=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
              x_tmp(nnew,2)=(xg(ind_grid(i),2)+xc(ind,2)-skip_loc(2))*scale
              x_tmp(nnew,3)=(xg(ind_grid(i),3)+xc(ind,3)-skip_loc(3))*scale
              dens_tmp(nnew)=uold(ind_cell(i),1)
              flag_tmp(nnew)=1
              point2flag2(ninc+ioffsetthread)=ind_cell(i) 
              ! This is a local pointer that is not shared with other cpus
#endif
           end if
        end do
...
!$omp parallel do private(i,x,y,z,j,dxx,dr_sink,dyy,dzz) schedule(dynamic)
  ...
  ntot_myid=ntot
  ntot=0
!!$omp parallel do private(i) reduction(+:ntot)
  do i=1,ntot_myid
...
! print *, myid, nsink, ntot_all, index_sink, index_sink_tot
...
!$omp parallel do private(igrid,ngrid,ind,iskip,i) reduction(+:nnew) schedule(static)
  ...
  mnew = nnew
  if(nnew .ge.1) then
     allocate(ind_grid_new(1:nnew), ind_cell_new(1:nnew), ind_part_cloud(1:nnew))
     allocate(ind_grid_cloud(1:nnew))
     allocate(ok_true(1:nnew))
     allocate(ind_ind(1:nnew))
  endif
     ...
     do igrid=1,ncache,nvector
      ...
      ! Gather cells with a new sink
          do i=1,ngrid
             if (flag2(ind_cell(i))>0)then
                nnew=nnew+1
                ind_grid_new(nnew+ioffsetthread)=ind_grid(i)
                ind_grid_cloud(nnew+ioffsetthread)=ind_grid(i)
                ind_cell_new(nnew+ioffsetthread)=ind_cell(i)
                ind_ind     (nnew+ioffsetthread)=ind
             end if
          end do
        enddo
     enddo
  endif
! call mpi_barrier(MPI_COMM_WORLD, info) if(verbose) print *,'### 1'
  if(mnew .ge. 1) then
     ok_true = .true.
     call remove_free(ind_part_cloud,mnew)
     call add_list(ind_part_cloud, ind_grid_cloud, ok_true, mnew)
  endif
! call mpi_barrier(MPI_COMM_WORLD, info) if(verbose) print *,'### 2'
...
!$omp          firstprivate(index_sink,index_sink_tot) 
      ...
      index_sink = index_sink + ioffsetthread     ! Thread shift
      index_sink_tot = index_sink_tot + ioffsetthread     ! Thread shift
   ! Create new sink particles
       do i=ioffsetthread + 1 ,ioffsetthread + nnew
          index_sink=index_sink+1
          index_sink_tot=index_sink_tot+1
...
!$omp end parallel 
       ! End loop over new sink particle cells
     deallocate(ind_grid_new, ind_cell_new, ind_part_cloud)
     deallocate(ind_grid_cloud)
     deallocate(ok_true)
     deallocate(ind_ind)
  endif
#endif
...
! call mpi_barrier(MPI_COMM_WORLD, info) if(verbose) print *,'### 3'
  !---------------------------------------------------------
...
! call mpi_barrier(MPI_COMM_WORLD, info) if(verbose) print *,'### 4'
  ...
  deallocate(Pok)
  deallocate(iokthread)
#endif

end subroutine kjhan_make_sink
!################################################################
!################################################################
!################################################################
!################################################################
subroutine merge_sink(ilevel)
  ...
  integer::igrp,jgrp,icomp,gndx,ifirst,ilast,indx
!#ifndef _OPENMP
  integer,dimension(1:nvector),save::ind_grid,ind_part,ind_grid_part
!#else
!  integer,target, allocatable::Pind_grid(:,:),Pind_part(:,:),Pind_grid_part(:,:)
!  integer, pointer:: ind_grid(:), ind_part(:), ind_grid_part(:)
!  common /omp_merge_sink_ind/ ind_grid, ind_part, ind_grid_part
!!$omp threadprivate(/omp_merge_sink_ind/)
!#endif
...
!  allocate(Pind_grid(1:nvector,0:nthreads-1), Pind_part(1:nvector,0:nthreads-1))
...
#ifdef _OPENMP
  ngrp = new_sink
  allocate(GroupData(1:ngrp, 1:2))
  do igrp=1,ngrp
     GroupData(igrp,1) = nsink+1
     GroupData(igrp,2) = 0
  enddo
  do ijsink=1,nsink
     igrp = gsink(psink(ijsink))
     GroupData(igrp,1) = MIN(ijsink, GroupData(igrp,1))
     GroupData(igrp,2) = MAX(ijsink, GroupData(igrp,2))
  enddo
!$omp parallel do private(igrp,ijsink,isink,M1,M2,a1,a2,ax1,ay1,az1, &
...
!$omp parallel do private(isink,xx,yy,zz)
  do isink=1,nsink
     xx=xsink(isink,1)
...
! call MPI_BARRIER(MPI_COMM_WORLD, info) if(verbose) print *,'###6'
  !-----------------------------------------------------
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart2>0)then
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then
                 call kjhan_kill_sink(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
...
!################################################################
!################################################################
subroutine kjhan_kill_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
  ...
  do j=1,np
     ...
     if(.not. ok(j))then
   ksink_new=oksink_all(ksink)
   idp(ind_part(j))=-ksink_new
   mp(ind_part(j))=msink(ksink_new)
   xp(ind_part(j),1)=xsink(ksink_new,1)
   vp(ind_part(j),1)=vsink(ksink_new,1)
#if NDIM>1
   xp(ind_part(j),2)=xsink(ksink_new,2)
   vp(ind_part(j),2)=vsink(ksink_new,2)
#endif
#if NDIM>2
   xp(ind_part(j),3)=xsink(ksink_new,3)
   vp(ind_part(j),3)=vsink(ksink_new,3)
#endif
     endif
  end do
...
!################################################################
!################################################################
subroutine kjhan_create_cloud(ilevel)
  use pm_commons
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
  ...
  common /omp1_threads/ mythread
!$omp threadprivate(/omp1_threads/)
  common /omp_c_c/ ind_grid, ind_grid_part, ind_part
!$omp threadprivate(/omp_c_c/)
#endif
...
!$omp end parallel
  ...
  ! Loop over cpus
  do icpu=1,ncpu
     if(numbl(icpu,ilevel).eq.0) goto 101
     igrid=headl(icpu,ilevel)
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(igrid,jgrid,npart1,npart2,npart3,ipart, jpart,next_part,ip,ig) 
     ig=0
     ip=0
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart2>0)then
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then
                 call kjhan_mk_cloud(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
...
111 format('  +Entering kjhan_create_cloud for level ',I2)

end subroutine kjhan_create_cloud
!################################################################
!################################################################
!################################################################
!################################################################
subroutine kjhan_mk_cloud(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use cooling_module, ONLY: XH=>X, rhoc, mH
...
!$omp end critical
       ...
       vp(ind_cloud(j),1)=vsink(ksink,1)
#if NDIM>1
       xp(ind_cloud(j),2)=xp(ind_part(j),2)+yy
       vp(ind_cloud(j),2)=vsink(ksink,2)
#endif
#if NDIM>2
       xp(ind_cloud(j),3)=xp(ind_part(j),3)+zz
       vp(ind_cloud(j),3)=vsink(ksink,3)
#endif
         end do
      end if
   end do
  ...
  do j=1,np
     ksink=-idp(ind_part(j))
     mp(ind_part(j))=msink(ksink)/dble(ncloud)
     vp(ind_part(j),1)=vsink(ksink,1)
     vp(ind_part(j),2)=vsink(ksink,2)
     vp(ind_part(j),3)=vsink(ksink,3)
  end do

end subroutine kjhan_mk_cloud
!################################################################
!################################################################
!################################################################
!################################################################
subroutine kjhan_kill_cloud(ilevel)
  use pm_commons
  use amr_commons
  implicit none
  integer::ilevel
  ...
  integer:: nthreads,mythread,npart3
  common /omp1_threads/ mythread
  common /omp_k_c/ind_part, ind_grid, ind_grid_part
!$omp threadprivate(/omp1_threads/)
!$omp threadprivate(/omp_k_c/)
#endif
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid,npart1, npart2,ipart,jpart,next_part,ip,ig) 
     ig=0
     ip=0
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart2>0)then
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then
                 call kjhan_rm_cloud(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
     ...
     ! End loop over grids
     if(ip>0)call kjhan_rm_cloud(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
!$omp end parallel
102  end do

111 format('  +Entering kjhan_kill_cloud for level ',I2)
#ifdef _OPENMP
  deallocate(nparticles, ptrhead)
  deallocate(Pind_grid, Pind_grid_part, Pind_part)
#endif

end subroutine kjhan_kill_cloud
!################################################################
!################################################################
!################################################################
!################################################################
subroutine kjhan_rm_cloud(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use cooling_module, ONLY: XH=>X, rhoc, mH
...
! integer,target, allocatable::Pind_grid(:,:),Pind_part(:,:),Pind_grid_part(:,:)
! integer, pointer:: ind_grid(:), ind_part(:), ind_grid_part(:)
  integer, dimension(1:nvector):: ind_part,ind_grid_part, ind_grid
...
!$omp              isink, r2,ip,ig,ksink) 
     ...
     do jgrid=1,npart3
       ...
       if(npart2>0)then
          ...
          do jpart=1,npart1
           ...
           if(ip==nvector)then
             call kjhan_bondi_velocity(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
             ip=0
             ig=0
           end if
           ipart=next_part  ! Go to next particle
    ...
    ! End loop over grids
    if(ip>0)call kjhan_bondi_velocity(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
!$omp end parallel 
102  end do
  ! End loop over cpus
...
!################################################################
!################################################################
subroutine kjhan_bondi_velocity(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use cooling_module, ONLY: XH=>X, rhoc, mH
...
end subroutine kjhan_bondi_velocity
!################################################################
!################################################################
!################################################################
!################################################################
...
!$omp parallel do
!private(isink,density,c2mean,velocity,volume,i,v2mean,alpha,ZZ1,ZZ2,r_lso,epsilon_r)
  do isink=1,nsink
     density=0d0
     c2mean=0d0
     velocity=0d0
...
!!$omp parallel do private(isink)
  ...
  ! YDspin
  if(spin_bh)call kjhan_growspin(ilevel)
  ! YDspin
  ...
  do j=1,np
     if(ok(j))then
        ...
        if(drag)then
           ...
           factor=alpha * fudge * d*msink(ksink)/cgas**2 / vnorm_rel
           ...
           factor=MIN(factor,1d0/dtnew(ilevel))
           do idim=1,ndim
...
subroutine grow_jeans(ilevel)
  ...
  integer,target, allocatable::Pind_grid(:,:),Pind_part(:,:),Pind_grid_part(:,:)
  integer,pointer::ind_grid(:),ind_part(:),ind_grid_part(:)
  common /omp_g_j/ ind_grid, ind_part, ind_grid_part
!$omp threadprivate(/omp_g_j/)
#endif
  real(dp)::r2,density,volume
...
!$omp end parallel
  ...
  if(ilevel .lt. nlevelmax_loc) then
  deallocate(nparticles,ptrhead)
  return
  endif
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid,npart1,npart2,ipart,next_part,ip,ig)
     ig=0
     ip=0
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
   ...
   if(npart2>0)then
      ...
      do jpart=1,npart1
       ...
       ind_grid_part(ip)=ig
         ...
         if(ip==nvector)then
       call kjhan_accrete_jeans(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
       ip=0
       ig=0
         end if
         ipart=next_part  ! Go to next particle
...
!################################################################
!################################################################
subroutine kjhan_accrete_jeans(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use cooling_module, ONLY: XH=>X, rhoc, mH, twopi
  ...
  integer(i8b)::ksink
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_m
  real(dp)::xxx,mmm,r2,v2,c2,d,u,v=0d0,w=0d0,e,bx1,bx2,by1,by2,bz1,bz2,z,oxy,iron
  real(dp)::dx,dx_loc,scale,vol_loc,temp,d_jeans,acc_mass,dd_sink,d_thres,nCOM
  real(dp)::factG,pi
  logical::error
#ifndef _OPENMP
  ! Grid based arrays
  ...
  if(error)then
     write(*,*)'problem in kjhan_accrete_jeans'
     write(*,*)ilevel,ng,np
     stop
  end if
...
!$omp critical
  do j=1,np
   ...
   if(d .ge. d_thres)then
      ...
      msink_new(ksink  )=msink_new(ksink  )+acc_mass
      vsink_new(ksink,1)=vsink_new(ksink,1)+acc_mass*u
#if NDIM>1
      vsink_new(ksink,2)=vsink_new(ksink,2)+acc_mass*v
#endif
#if NDIM>2
...
end subroutine kjhan_accrete_jeans
!################################################################
!################################################################
!################################################################
!################################################################
subroutine kjhan_jet_AGN(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
 ...
 ! Lower left corner of 3x3x3 grid-cube
  ...
  if(error)then
     write(*,*)'problem in kjhan_jet_AGN'
     write(*,*)ilevel,ng,np
     stop
  end if
...
!$omp critical
  do j=1,np
     if(ok(j))then
        ...
        d_x=xp(ind_part(j),1)-xsink(ksink,1)
        d_y=xp(ind_part(j),2)-xsink(ksink,2)
        d_z=xp(ind_part(j),3)-xsink(ksink,3)
        ...
        jsink_new(ksink,1)=jsink_new(ksink,1) + (d_y*wc-d_z*vc)*d*vol_loc
        jsink_new(ksink,2)=jsink_new(ksink,2) + (d_z*uc-d_x*wc)*d*vol_loc
        jsink_new(ksink,3)=jsink_new(ksink,3) + (d_x*vc-d_y*uc)*d*vol_loc
...
end subroutine kjhan_jet_AGN
!##############################################################################
!##############################################################################
!##############################################################################
!##############################################################################
subroutine kjhan_get_rho_star(ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
...
end subroutine kjhan_get_rho_star
!##############################################################################
!##############################################################################
!##############################################################################
!##############################################################################
subroutine rhostar_from_current_level(ilevel)
  ...
  integer,target, allocatable:: Pind_part(:,:), Pind_grid_part(:,:)
  integer,pointer:: ind_part(:), ind_grid_part(:)

  real(dp),target, allocatable:: Px0(:,:,:)
  real(dp), pointer:: x0(:,:) 
  common /openmp_g_r_s/ ind_grid,ind_cell,ind_part,ind_grid_part, x0
!$omp threadprivate(/openmp_g_r_s/)
  integer:: mythread, nthreads, npart3
  common /openmp_threads1/ mythread
  integer, dimension(:), allocatable:: nparticles, ptrhead
!$omp threadprivate(/openmp_threads1/)
...
!$omp end parallel
  allocate(Pind_part(1:nvector, 0: nthreads-1))
  allocate(Pind_cell(1:nvector, 0: nthreads-1))
  allocate(Pind_grid(1:nvector, 0: nthreads-1))
  allocate(Pind_grid_part(1:nvector, 0: nthreads-1))
  allocate(Px0(1:nvector, 1:ndim, 0: nthreads-1))
  allocate(nparticles(0:nthreads-1), ptrhead(0:nthreads-1))
!$omp parallel
  ind_part => Pind_part(:, mythread)
  ind_grid => Pind_grid(:, mythread)
...
!$omp end parallel
  ...
  ! Loop over cpus
  do icpu=1,ncpu
!    if(numbl(icpu,ilevel).le.0) goto 11
     igrid=headl(icpu,ilevel)
     npart3 = numbl(icpu,ilevel)
#ifndef OMP_TEST
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid,npart1,ipart,ip,ig,idim,i,jpart)
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
#endif
     ig=0
...
#ifdef _OPENMP
  deallocate(nparticles, ptrhead)
  deallocate(Pind_part)
  deallocate(Pind_cell)
  deallocate(Pind_grid)
  deallocate(Pind_grid_part)
  deallocate(Px0)
#endif
...
!###########################################################
!###########################################################
subroutine kjhan_quenching(ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
...
111 format('  +Entering kjhan_quenching for level ',I2)

end subroutine kjhan_quenching
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine kjhan_growspin(ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
  ...
  !----------------------------------------------------------------------
  ! local constants
  integer::isink,nisink,njsink,nwsink,ierr,nid,i,j
  integer, allocatable, dimension(:):: displs, recvcnt
  real(dp), allocatable, dimension(:):: lspinmag, leps_sink,works,workr
  integer:: ilevel,iflag
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_m
  real(dp)::onethird,epsilon_r,alph=0.1d0,nu_ratio,fourpi,prefact
  real(dp)::mbh,dmcoarse,mdisc,dm,chi,ax2,ay2,az2,jsinknorm,amod,amod2,mass_8,t_nu1
  real(dp)::rwarp,rsg,msg,dmacc,ax1,ay1,az1,bhspinnorm,theta,reduction_factor
  ...
  if(verbose)write(*,*)' +Entering kjhan_growspin2'
  ...
  nid = ncpu
  if(ilevel .lt. nlevelmax .or. nsink .lt. nid*10) then
    nisink = 1
    njsink = nsink
    iflag = 0
  else
    allocate(displs(1:nid), recvcnt(1:nid))
    call  mpiworksplit(nid,myid,nsink,nisink,njsink,nwsink, displs,recvcnt)
    allocate(lspinmag(1:nsink), leps_sink(1:nsink))
    iflag = 1
  endif

! nwsink = (nsink + nid-1)/nid
! nisink = nwsink * (myid-1) + 1
! njsink = nwsink * (myid  )
! if(nisink .gt. nsink) then
!    nisink = 0
!    njsink = 0 
!    nwsink = 0
! else if (nisink .le. nsink .and. njsink .gt. nsink) then
!    njsink = nsink
!    nwsink = (njsink-nisink)+1
! endif
! if(myid.eq.1) then
!    do i = 1, nid
!       displs(i) = nwsink * (i-1)
!       recvcnt(i) = nwsink
!       if(displs(i).gt. nsink) then
!           recvcnt(i) = 0
!       else if (displs(i) + recvcnt(i) .gt. nsink) then
!           recvcnt(i) = nsink - displs(i)
!       endif
!    enddo
! endif
...
!$omp parallel do private(isink,mdisc,dm,chi,dmcoarse, dmini,mbh,ax2,ay2,az2,&
!$omp                 jsinknorm, amod,ZZ1,ZZ2,r_lso,epsilon_r, chieps,mass_8,&
!$omp                 t_nu1,rwarp,rsg,msg,dmacc,ax1,ay1,az1,bhspinnorm,theta,&
!$omp                 aac,jbh, jd,amod2,dmdeplete,mbh2,spinup,dm2) schedule(dynamic)
  do isink=nisink,njsink
  ...
  call MPI_Gatherv(spinmag(nisink),nwsink, MPI_DOUBLE_PRECISION, lspinmag, recvcnt,displs, &
                &   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_Bcast(lspinmag, nsink, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_Gatherv(eps_sink(nisink),nwsink, MPI_DOUBLE_PRECISION, leps_sink, recvcnt,displs, &
                &   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_Bcast(leps_sink, nsink, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)

  allocate(works(1:nsink*ndim),workr(1:nsink*ndim))
  do i = 1, ndim
  do j = nisink, njsink
     works(1+ndim*(j-1)) = bhspin(j,i)
  enddo
  enddo
  call MPI_Gatherv(works(1+(nisink-1)*ndim),ndim*nwsink, MPI_DOUBLE_PRECISION, workr,&
                &   recvcnt*ndim,displs*ndim, &
                &   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_Bcast(workr, ndim*nsink, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
!$omp parallel do private(isink)
  do isink = 1, nsink
     spinmag(isink) = lspinmag(isink)
     eps_sink(isink) = leps_sink(isink)
     do i = 1, ndim
        bhspin(j,i) = workr(i+ndim*(j-1))
     enddo
  enddo

  deallocate(works,workr)
  deallocate(lspinmag, leps_sink)
  deallocate(displs, recvcnt)
  endif

end subroutine kjhan_growspin
!###########################################################
!###########################################################
!###########################################################
!###########################################################
...
! logical ,dimension(1:nvector)::ok
  ...
  logical ,target, allocatable, dimension(:,:)::Pok
  integer,pointer,  dimension(:)::ind_grid,ind_cell
  logical ,pointer,  dimension(:)::ok
  common /com_average_AGN/ ind_grid,ind_cell,ok
!$omp threadprivate(/com_average_AGN/)
  integer:: mythread, nthreads
  common /omp_threads_agn/ mythread
!$omp threadprivate(/omp_threads_agn/)
  real(dp),target, allocatable, dimension(:,:)::Pvol_gas,Pmass_gas,Ppsy_norm
  real(dp),pointer,  dimension(:)::Tvol_gas,Tmass_gas,Tpsy_norm
  common /com_av_agn/ Tvol_gas,Tmass_gas,Tpsy_norm
!$omp threadprivate(/com_av_agn/)
...
!$omp end parallel
  ...
  allocate(Pind_grid(1:nvector, 0: nthreads-1))
  allocate(Pok(1:nvector, 0: nthreads-1))
  allocate(Pvol_gas(1:nAGN, 0: nthreads-1))
  allocate(Pmass_gas(1:nAGN, 0: nthreads-1))
  allocate(Ppsy_norm(1:nAGN, 0: nthreads-1))
!$omp parallel
  ind_grid => Pind_grid(:, mythread)
  ind_cell => Pind_cell(:, mythread)
  ok => Pok(:, mythread)
  Tvol_gas => Pvol_gas(:, mythread)
  Tmass_gas => Pmass_gas(:,  mythread)
  Tpsy_norm => Ppsy_norm(:, mythread)
!$omp end parallel
...
! vol_gas, mass_gas, psy_norm: global return variables of iAGN and possibly added several times
                     ...
                     if(X_radio(iAGN).lt.X_floor)then
                       ...
                       if(ok_blast_agn(iAGN))then
                        ...
                        ! ------------------------------------------
                         ...
                         if(dr_AGN.le.rmax2)then
                            Tvol_gas (iAGN)=Tvol_gas (iAGN)+vol_loc*uold(ind_cell(i),1)
                            Tmass_gas(iAGN)=Tmass_gas(iAGN)+vol_loc*uold(ind_cell(i),1)
                         endif
                         dr_cell=MAX(ABS(dxx),ABS(dyy),ABS(dzz))
                         if(dr_cell.le.dx_loc/2.0)then
                            ind_blast(iAGN)=ind_cell(i)
...
!$omp parallel do private(iAGN)
  do iAGN = 1, nAGN
  vol_gas(iAGN) = sum(Pvol_gas(iAGN,0:nthreads-1))
  mass_gas(iAGN) = sum(Pmass_gas(iAGN,0:nthreads-1))
...
subroutine AGN_blast(xAGN,vAGN,dMsmbh_AGN,dMBH_AGN,dMEd_AGN,mAGN,ZAGN,jAGN,ind_blast,vol_gas &
  ...
  real(dp),dimension(1:3)::xbound,skip_loc
  real(dp),dimension(1:twotondim,1:3)::xc
  real(dp),dimension(1:nAGN)::mAGN,ZAGN,p_gas,vol_gas,uBlast,vol_blast
  real(dp),dimension(1:nAGN)::EAGN,X_radio,psy_norm,dMsmbh_AGN,dMBH_AGN,dMEd_AGN,spinmagAGN,eps_AGN

  real(dp),dimension(1:nAGN)::EsaveAGN
  ...
  logical, pointer:: ok(:)
  integer,pointer::ind_grid(:),ind_cell(:)
  common /omp_a_b/ ok, ind_grid, ind_cell, TEsaveAGN
!$omp threadprivate(/omp_a_b/)
  integer mythread, nthreads,nwork,icount,jcount
  common /omp_threads_agn_blast/ mythread
...
!$omp end parallel
  allocate(Pind_grid(1:nvector,0:nthreads-1), Pind_cell(1:nvector,0:nthreads-1))
  allocate(Pok(1:nvector,0:nthreads-1))
  allocate(PEsaveAGN(1:nAGN,0:nthreads-1))
!$omp parallel
  ind_grid => Pind_grid(:, mythread)
  ind_cell => Pind_cell(:, mythread)
  ok => Pok(:, mythread)
  TEsaveAGN=>PEsaveAGN(:, mythread)
...
!###############################################################################################
!    uold(ind_cell(i)) is thread-safe.
!    EsaveAGN(iAGN) is thread-unsafe and, therefore, needs to have additional thread dimension.
!###############################################################################################
     ! Loop over grids
     ncache=active(ilevel)%ngrid
...
!################################################################
!################################################################
subroutine kjhan_getAGNonmyid(isink_myid,nsink_myid)
  use amr_commons
  use pm_commons
  implicit none
#ifndef WITHOUTMPI
  ...
  isink_myid=0
  ii=0
!$omp parallel do private(isink,cpu_read,xxmin,xxmax,yymin,yymax,zzmin,zzmax,dmax,dx,lmin,bit_length,maxdom, &
!$omp   imin,imax,jmin,jmax,kmin,kmax,dkey,ndom,idom,jdom,kdom,i,order_min,bounding_min,bounding_max,cpu_min,cpu_max, &
!$omp   impi,ncpu_read,j, cpu_list, icpu)
  do isink=1,nsink
     cpu_read=.false.
     ! Compute boundaries for the sink cube of influence
     xxmin=(xsink(isink,1)-drsink)/scale ; xxmax=(xsink(isink,1)+drsink)/scale
  ...
  ii = 0
  do isink = 1, nsink
    if(isink_myid(isink) .ne.0) then
       ii = ii + 1
       isink_myid(ii) = isink_myid(isink)
    endif
  enddo
...
end subroutine kjhan_getAGNonmyid
!################################################################
!################################################################
!################################################################
!################################################################
subroutine kjhan_update_sink_position_velocity
  use pm_commons
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
...
#endif
!$omp parallel do private(isink,ncloud,idim,vdum)
  do isink=1,nsink
...
end subroutine kjhan_update_sink_position_velocity
