...
subroutine merge_tree_fine(ilevel)
  ...
  logical,dimension(1:nvector),save::ok
#else
  integer, target, allocatable::Pind_grid(:,:),Pind_cell(:,:),Pind_grid_son(:,:)
  integer, pointer::ind_grid(:), ind_cell(:), ind_grid_son(:)
  logical, target, allocatable::Pok(:,:)
  logical, pointer:: ok(:)
  common /openmp_m_t_fine/ ind_grid, ind_cell, ind_grid_son, ok
!$omp threadprivate(/openmp_m_t_fine/)
  integer:: mythread, nthreads
...
!$omp parallel
  ...
  nthreads = Omp_get_num_threads()
!$omp end parallel
  allocate(Pind_cell(1:nvector,0:nthreads-1),Pind_grid_son(1:nvector,0:nthreads-1),Pok(1:nvector,0:nthreads-1))
  allocate(Pind_grid(1:nvector,0:nthreads-1))
!$omp parallel
  ind_grid => Pind_grid(:,mythread)
  ind_cell => Pind_cell(:,mythread)
  ind_grid_son => Pind_grid_son(:,mythread)
  ok => Pok(:,mythread)
!$omp end parallel
#endif
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=tpp(ind_part(i))
        reception(icpu,ilevel)%up(ind_com(i),current_property+1)=mp0(ind_part(i))
        reception(icpu,ilevel)%up(ind_com(i),current_property+2)=indtab(ind_part(i))
!        do ielt=1,nelt
!           reception(icpu,ilevel)%up(ind_com(i),current_property+2+ielt)=cep(ind_part(i),ielt)
!        end do
     end do
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     do i=1,np
        tpp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
        mp0(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+1)
        indtab(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+2)
!        do ielt=1,nelt
!           cep(ind_part(i),ielt)=emission(icpu,ilevel)%up(ind_com(i),current_property+2+ielt)
!        enddo
     end do
     current_property = current_property+3!+nelt
  end if
