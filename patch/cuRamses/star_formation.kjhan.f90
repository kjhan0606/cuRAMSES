...
! if(verbose)write(*,*)' +Entering star_formation:1'
...
!$omp parallel do private(igrid,ngrid) 
  do igrid=1,ncache,nvector
	 ...
	 call sub1_star_formation(ilevel, igrid,ngrid)
  end do
! call MPI_Barrier(MPI_COMM_WORLD, info)
! if(verbose)write(*,*)' +Entering star_formation:2'
...
#endif
! call MPI_Barrier(MPI_COMM_WORLD, info)
! if(verbose)write(*,*)' +Entering star_formation:3'
  ...
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!!$omp parallel do private(igrid,ngrid,intot,imstar_lost,imstar_tot) reduction(+:jntot,jmstar_lost,jmstar_tot)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
	 call sub2_star_formation(ilevel, igrid,ngrid,intot,imstar_lost, imstar_tot)
	 jntot = jntot + intot
	 jmstar_lost = jmstar_lost + imstar_lost
...
! call MPI_Barrier(MPI_COMM_WORLD, info)
! if(verbose)write(*,*)' +Entering star_formation:5'
...
! call MPI_Barrier(MPI_COMM_WORLD, info)
! if(verbose)write(*,*)' +Entering star_formation:6'
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           tpp(ind_part(i))=texp ! Birth epoch proper
           mp0(ind_part(i))=n*mstar

           if(sf_birth_properties) then
              if(sf_virial)then
                 !writing to binary starlog
                 write(ilun)index_star
                 write(ilun)ilevel
                 write(ilun)birth_epoch
                 write(ilun)texp
                 write(ilun)n*mstar
                 write(ilun)x
                 write(ilun)y
                 ...
                 write(ilun)tg
                 write(ilun)zg
                 write(ilun)uold(ind_cell_new(i),ivirial)
                 do ielt=1,nelt
                    write(ilun)uold(ind_cell_new(i),ichem+ielt-1)
                 enddo
              else
                 !writing to binary starlog
                 write(ilun)index_star
  ...
  ! End loop over grids
! call MPI_Barrier(MPI_COMM_WORLD, info)
! if(verbose)write(*,*)' +Entering star_formation:7'
...
!$omp parallel do private(igrid,ngrid)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
	 call sub3_star_formation(ilevel, igrid,ngrid)
  end do

#endif
  if(sf_birth_properties) close(ilun)
! call MPI_Barrier(MPI_COMM_WORLD, info)
! if(verbose)write(*,*)' +Entering star_formation:8'
     ...
     do i=1,ngrid
        ...
        if(ok(i))then
           ...
           if (mgas > 0.9*mcell) then
              ...
              imstar_lost=imstar_lost+(nstar(i)-nstar_corrected)*mstar
              nstar(i)=nstar_corrected
           endif
           ! Compute new stars local statistics
           imstar_tot=imstar_tot+nstar(i)*mstar
           if(nstar(i)>0)then
              ntot=ntot+1
           endif
        endif
...
end subroutine sub2_star_formation
!###################################################
!###################################################
!###################################################
!###################################################
...
end subroutine sub3_star_formation
