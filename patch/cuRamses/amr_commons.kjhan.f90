module amr_commons
  ...
  integer(i8b),allocatable,dimension(:)::bisec_cpu_load     ! CPU loads (for stats)
  ...
  ! K-section decomposition (generalized recursive bisection)
  integer :: nksec_levels = 0                                ! total split levels
  integer, allocatable, dimension(:) :: ksec_factor          ! branching factor per level (1:nksec_levels)
  integer, allocatable, dimension(:) :: ksec_dir             ! split direction per level (1:nksec_levels)
  integer :: ksec_kmax = 2                                   ! max branching factor
  integer :: ksec_nbinodes = 0                               ! total tree nodes for ksection

  ! K-section tree arrays (generalized from bisection binary tree)
  real(dp), allocatable, dimension(:,:) :: ksec_wall         ! (ksec_nbinodes, 1:ksec_kmax-1) walls per node
  integer,  allocatable, dimension(:,:) :: ksec_next         ! (ksec_nbinodes, 1:ksec_kmax) children per node
  integer,  allocatable, dimension(:)   :: ksec_indx         ! leaf -> CPU id
  integer :: ksec_root = 0

  ! K-section tree navigation for hierarchical exchange
  integer, allocatable, dimension(:) :: ksec_cpumin     ! (1:ksec_nbinodes) min CPU id in subtree
  integer, allocatable, dimension(:) :: ksec_cpumax     ! (1:ksec_nbinodes) max CPU id in subtree
  integer, allocatable, dimension(:,:) :: ksec_cpu_path ! (1:ncpu, 1:nksec_levels) child index per level
