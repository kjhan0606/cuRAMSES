...
% ------------------------------------------------------------------
\paramheader{jobcontrolfile}{character(128)}{'' (empty)}
\label{param:jobcontrolfile}

\begin{quote}
Path to a runtime job control file.  When set to a non-empty string,
rank~0 reads this file at every coarse step to check for user-requested
actions.  The action is broadcast to all ranks via \texttt{MPI\_BCAST}.

\textbf{File format.}\quad  Each line contains two integers separated by
whitespace:
\begin{center}
\texttt{step\_number\quad action\_code}
\end{center}
\begin{itemize}[nosep]
  \item \texttt{step\_number = 0} --- match immediately (every step).
  \item \texttt{step\_number =} $N$ --- match when
        \texttt{nstep\_coarse} $= N$.
  \item \texttt{action\_code = 0} --- do nothing.
  \item \texttt{action\_code = 1} --- write an extra snapshot and continue.
  \item \texttt{action\_code = -1} --- write an extra snapshot and stop
        gracefully (sets \paramref{nstepmax}\texttt{= nstep\_coarse}).
\end{itemize}
If multiple lines match, \texttt{-1} (stop) takes priority over
\texttt{1} (output).  The file is \emph{not} deleted after reading;
lines with \texttt{step\_number = 0} re-trigger every step until the
file is removed or modified by the user.

\textbf{Example.}\quad  To request a graceful stop at the next coarse step:
\begin{verbatim}
...
\apinote{Output numbering (\texttt{ifout}) continues incrementally;
extra outputs from job control use the next sequential number
automatically.}
\end{quote}
...
The following parameters control GPU acceleration.  They require
compilation with \texttt{make USE\_CUDA=1}; when compiled without CUDA
support, any \texttt{gpu\_*} flags set to \texttt{.true.}\ are
silently reset to \texttt{.false.}\ with a warning.

All three flags default to \texttt{.false.}\ (CPU-only mode).  They
can be combined independently to select the optimal configuration for
the available hardware.

% ------------------------------------------------------------------
\paramheader{gpu\_hydro}{logical}{.false.}
\label{param:gpu_hydro}

\begin{quote}
Enable hybrid CPU/GPU hydrodynamics solver.  When \texttt{.true.}, the
Godunov solver dispatches large grid batches to the GPU while small
batches are processed by CPU threads.  The full mesh
(\texttt{uold}, \texttt{f}, \texttt{son}) is uploaded to GPU memory
once per \texttt{godunov\_fine} call ($\sim$19\,GB for a typical
$512^3$ run with 11 hydro variables).

\textbf{Requires one MPI rank per GPU.}

\apinote{On GPUs with limited PCIe bandwidth (e.g.\ NVIDIA A40 via
PCIe Gen4), the mesh upload and gather/scatter overhead can exceed the
GPU compute benefit, resulting in a net slowdown.  Benchmark before
enabling in production.  The code automatically falls back to CPU-only
if GPU memory is insufficient.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{gpu\_poisson}{logical}{.false.}
\label{param:gpu_poisson}

\begin{quote}
Enable GPU-accelerated Poisson multigrid (MG) V-cycle for AMR levels.
Gauss--Seidel red--black smoothing and residual computation run on the
GPU; halo exchanges use pinned-memory asynchronous transfers.

The GPU MG solver uploads $\sim$6.4\,GB of MG arrays (phi, f, flag2,
nbor, igrid) plus $\sim$320\,MB for GPU restrict/interpolation
buffers.

\apinote{Like \paramref{gpu_hydro}, this feature is PCIe-bandwidth
limited.  Benchmarks on A40 (PCIe Gen4) show GPU MG is slower than
CPU MG.  On GPUs with high-bandwidth host--device links (e.g.\ Grace
Hopper), GPU MG may provide a benefit.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{gpu\_fft}{logical}{.false.}
\label{param:gpu_fft}

\begin{quote}
Enable cuFFT direct Poisson solve for fully uniform AMR levels.  When
a level is fully covered (all $N = n_x \times n_y \times n_z \times
8^{\ell-1}$ grids exist), the Poisson equation is solved via 3D FFT
instead of multigrid V-cycles.  This bypasses iterative convergence
entirely.

The solver flow is:
\begin{enumerate}[nosep]
  \item Gather local cell RHS values into a global 3D array via
        \texttt{MPI\_ALLREDUCE}.
  \item Upload to GPU and execute forward R2C FFT (cuFFT).
  \item Apply the spectral Green's function
        $G(\mathbf{k}) = \Delta x^2 / (N^3 L_k)$
        where $L_k = 2\cos k_x + 2\cos k_y + 2\cos k_z - 6$.
  \item Inverse C2R FFT and download result to host.
  \item Scatter $\phi$ back to RAMSES cells.
\end{enumerate}

\textbf{GPU memory usage:} $\sim$3\,GB per rank for a $512^3$ grid.

\textbf{Performance:} For a $512^3$ base grid (4 MPI ranks, A40),
the cuFFT direct solve reduces the base-level Poisson time from
242\,s (MG V-cycle) to $\sim$20\,s (\textbf{12$\times$ speedup}).

\apinote{This flag is independent of \paramref{gpu_poisson}.  When
both are \texttt{.false.}, all Poisson solving uses CPU multigrid.
When only \texttt{gpu\_fft=.true.}, the base level uses cuFFT on GPU
while AMR levels use CPU multigrid --- this is often the optimal
configuration on PCIe-limited GPUs.}

\apiwarning{The current implementation uses \texttt{MPI\_ALLREDUCE}
to gather the global RHS array, which becomes a bottleneck for
$N > 256^3$ ($\sim$1\,GB of data).  For $256^3$ and below, the
overhead is negligible.}
\end{quote}

\noindent\textcolor{apidefault}{\textsf{\textbf{Examples:}}}
\begin{lstlisting}
...
% ------------------------------------------------------------------
\paramheader{use\_fftw}{logical}{.false.}
\label{param:use_fftw}

\begin{quote}
Enable FFTW3 CPU direct Poisson solve for fully uniform AMR levels.
This is the \textbf{CPU-only alternative} to \paramref{gpu_fft}:
it uses \textsc{fftw3} (MPI + OpenMP) instead of cuFFT, achieving
the same ${\sim}8\times$ speedup over multigrid V-cycles without
requiring a GPU.

Two grid-size paths are selected automatically:
\begin{itemize}[nosep]
  \item \textbf{Small grid} ($N \le 256^3$): \texttt{MPI\_ALLREDUCE}
        gathers the global RHS; a local FFTW3+OpenMP in-place R2C
        FFT solves the system.
  \item \textbf{Large grid} ($N > 256^3$): FFTW's MPI slab
        decomposition distributes the FFT across ranks.  Data
        redistribution between RAMSES's spatial domain and FFTW's
        $x$-slab layout uses \textbf{sparse point-to-point} exchange
        (\texttt{MPI\_ISEND}/\texttt{MPI\_IRECV}) with precomputed
        partner lists from \texttt{bisec\_cpubox}.  Only actual
        overlapping partners communicate (${\sim}5$--20 out of
        $N_{\text{cpu}}$), replacing the $O(N_{\text{cpu}})$
        \texttt{MPI\_ALLTOALLV} of the initial implementation.
\end{itemize}

Partner lists are cached and invalidated on load rebalancing.  The
overlap test uses the symmetric condition
$x_{\max}^{(A)} > (s_{\min}^{(B)} - 0.5) / N_x \;\wedge\;
x_{\min}^{(A)} < (s_{\max}^{(B)} + 1.5) / N_x$,
with periodic boundary wrapping, ensuring every \texttt{ISEND} has a
matching \texttt{IRECV}.

\textbf{Performance:} For a $512^3$ base grid (12 MPI ranks),
FFTW3 reduces base-level Poisson from 241\,s (MG) to 28.9\,s
(\textbf{8.3$\times$ speedup}), with $e_{\text{cons}} = 6.12 \times
10^{-4}$ (identical to cuFFT).

\textbf{Build requirement:} \texttt{make HDF5=1 USE\_FFTW=1}
(links \texttt{-lfftw3\_mpi -lfftw3\_omp -lfftw3}).

\apinote{This flag is mutually exclusive with \paramref{gpu_fft}.
If both are set, \texttt{use\_fftw} takes precedence.  Unlike
\texttt{gpu\_fft}, this requires no GPU and scales to arbitrarily
large grids via the sparse P2P slab exchange.}
\end{quote}

\noindent\textcolor{apidefault}{\textsf{\textbf{Examples:}}}
\begin{lstlisting}
! CPU-only FFTW3 for base level (recommended)
use_fftw = .true.

! Equivalent to gpu_fft but without GPU
! Requires: make HDF5=1 USE_FFTW=1
\end{lstlisting}

% ------------------------------------------------------------------
\paramheader{use\_neutrino}{logical}{.false.}
\label{param:use_neutrino}

\begin{quote}
Enable massive neutrino linear response in the Poisson solver.  When
\texttt{.true.}, the gravitational potential receives a scale-dependent
correction from neutrino density perturbations, computed via the CAMB
transfer function ratio $R_\nu(k,a) = T_\nu(k,a) / T_{\mathrm{cb}}(k,a)$.

\textbf{FFT solver path} (uniform levels with \paramref{use_fftw} or
\paramref{gpu_fft}): in Fourier space, the potential is modified as
\[
  \hat\phi(\mathbf{k}) \;\leftarrow\; \hat\phi(\mathbf{k}) \times
  \Bigl[1 + \frac{\Omega_\nu}{\Omega_{\mathrm{cb}}} R_\nu(k, a)\Bigr]
\]
where $\Omega_{\mathrm{cb}} = \Omega_m - \Omega_\nu$.

\textbf{Multigrid solver path} (AMR levels): the Poisson source term
uses $\Omega_{\mathrm{cb}}$ instead of $\Omega_m$:
\[
  4\pi G \to \tfrac{3}{2}\,\Omega_{\mathrm{cb}}\,a
\]
The neutrino contribution to the uniform background is already captured
by the Friedmann equation (where $\Omega_m$ includes neutrinos).

\textbf{Requires:}
\begin{itemize}[nosep]
  \item \paramref{omega_nu} $> 0$ in \texttt{\&NEUTRINO\_PARAMS}
  \item \paramref{neutrino_table} pointing to a valid CAMB table file
        in \texttt{\&NEUTRINO\_PARAMS}
\end{itemize}

If either is missing, the code prints a warning and disables neutrinos.

\apinote{Generate the transfer function table with
\texttt{python3 misc/generate\_neutrino\_table.py}.  This produces a
200~$k$-point $\times$ 50~$a$-point table using CAMB (default:
$\Sigma m_\nu = 0.06\,$eV).  When $\Omega_\nu = 0$, the code produces
bit-identical results to a run without this flag.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{de\_perturb}{logical}{.false.}
\label{param:de_perturb}

\begin{quote}
Enable dark energy density perturbations in the Poisson solver.
When \texttt{.true.}, the gravitational potential receives a
scale-dependent correction from dark energy perturbations, computed
via the CAMB transfer function ratio or the quasi-static
$\kappa^2/\alpha$ method.

Three modes of operation:
\begin{enumerate}[nosep]
  \item \paramref{de_table} provided $\to$ \textbf{table-based linear
    response} (works for any \paramref{cs2_de}, including~0).
  \item No \paramref{de_table}, \paramref{cs2_de}~$> 0$ $\to$
    \textbf{quasi-static $\kappa^2/\alpha$ method}.
  \item No \paramref{de_table}, \paramref{cs2_de}~$\leq 0$ $\to$
    unsupported; the code disables this flag with a warning.
\end{enumerate}

\textbf{Requires:}
\begin{itemize}[nosep]
  \item \paramref{cosmo}\texttt{=.true.}
  \item Either \paramref{de_table} or \paramref{cs2_de}~$> 0$
        in \texttt{\&CPL\_PARAMS}
  \item FFT solver active (\paramref{use_fftw} or \paramref{gpu_fft})
\end{itemize}

\apinote{Can be combined with \paramref{use_neutrino} --- both
corrections are applied multiplicatively in Fourier space.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{dump\_pk}{logical}{.false.}
\label{param:dump_pk}

\begin{quote}
When \texttt{.true.}, compute and write the matter power spectrum
$P(k)$ at each snapshot output.  The spectrum is computed via NGP
grid assignment on the base-level grid followed by an FFTW3 FFT.
The output file \texttt{pk\_NNNNN.dat} is written in the corresponding
\texttt{output\_NNNNN/} directory with columns:
$k$~[$h$/Mpc], $P(k)$~[($h^{-1}$Mpc)$^3$],
$P(k) - P_{\rm shot}$, and $N_{\rm modes}$.

\textbf{Requires} \paramref{use_fftw}\texttt{=.true.}\ or
\paramref{gpu_fft}\texttt{=.true.}\ (an FFT solver must be active).

\apinote{Useful for validating dark energy perturbation and neutrino
linear response implementations against CAMB linear theory
predictions.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm}{logical}{.false.}
\label{param:sidm}

\begin{quote}
Enable self-interacting dark matter (SIDM) Monte Carlo scattering.
When \texttt{.true.}, pairs of dark matter particles within the same
AMR cell undergo probabilistic elastic scattering with cross-section
$\sigma/m$ set by \paramref{sidm_cross_section} in
\texttt{\&SIDM\_PARAMS}.

\textbf{Requires} \paramref{pic}\texttt{=.true.}\ (particles must be
active).

\apinote{Scattering is computed cell-by-cell on AMR leaf cells.
Cells with fewer than \paramref{sidm_npart_min} dark matter particles
are skipped.  See Section~\ref{sec:sidm_params} for the full set of
SIDM parameters.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{use\_mond}{logical}{.false.}
\label{param:use_mond}

\begin{quote}
Enable Modified Newtonian Dynamics (MOND).  When \texttt{.true.}, the
gravitational force is modified according to the MOND prescription set
by \paramref{mond_type} in \texttt{\&MOND\_PARAMS}.  Three modes are
available: algebraic QUMOND (Phase~0), full QUMOND with phantom density
(Phase~1), and AQUAL iterative solver (Phase~2).

\textbf{Requires} \paramref{poisson}\texttt{=.true.}.

\apinote{A warning is issued if \paramref{pic}\texttt{=.true.}\ (dark
matter particles present), since MOND is designed to replace dark
matter.  See Section~\ref{sec:mond_params} for the full set of MOND
parameters.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{exchange\_method}{character}{`auto'}
\label{param:exchange_method}

\begin{quote}
MPI exchange method for virtual boundary (ghost zone) communication.
\begin{description}[style=nextline,leftmargin=2.5cm,font=\ttfamily]
  \item[`auto'] Adaptive auto-tuning.  The code benchmarks both methods
    during the first 50 calls for each of the 7 exchange components
    (fine/reverse/pair/bulk, dp/int), then selects the faster method.
    Periodic re-checks occur every 500 calls (with exponential backoff)
    to adapt to evolving load patterns.
  \item[`p2p'] Force point-to-point (\texttt{MPI\_ISEND/IRECV}) for all
    exchanges.
  \item[`ksection'] Force k-section tree routing for all exchanges.
\end{description}

\textbf{Requires} \paramref{ordering}\texttt{='ksection'} for the
\texttt{'ksection'} and \texttt{'auto'} options.

\apinote{The 7 exchange components tuned independently are:
\texttt{fine\_dp}, \texttt{fine\_int}, \texttt{reverse\_dp},
\texttt{reverse\_int}, \texttt{pair\_int}, \texttt{bulk\_dp}, and
\texttt{bulk\_rev\_dp}.  Auto-tuning decisions are synchronised across
all ranks via \texttt{MPI\_ALLREDUCE(SUM)} to ensure consistent
communication patterns.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{gpu\_sink}{logical}{.false.}
\label{param:gpu_sink}

\begin{quote}
Enable GPU-accelerated AGN feedback kernels (average\_AGN and
AGN\_blast).  When \texttt{.true.}, sink particle feedback operations
are offloaded to the GPU using flattened cell arrays and a 27-bin
spatial binning kernel.

\apinote{Benchmarks show CPU spatial binning is consistently faster than
the GPU path ($1.8\,$s vs $8.0\,$s on A100).  The GPU auto-tuning
framework (see Section~\ref{sec:run_params}) will correctly select the
CPU path in most cases.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{mem\_weight\_sink}{real(dp)}{500}
\label{param:mem_weight_sink}

\begin{quote}
Computational cost weight per sink particle for memory-based load
balancing.  When \paramref{memory_balance}\texttt{=.true.}, grids
containing sink particles receive an additional cost proportional to
this weight, so the bisection algorithm distributes sink-hosting grids
more evenly across MPI processes.

The total cell cost becomes:
\[
  C_{\text{cell}} = \frac{\texttt{mem\_weight\_grid}}{\texttt{twotondim}}
                + n_{\text{part}} \times
                  \frac{\texttt{mem\_weight\_part}}{\texttt{twotondim}}
                + n_{\text{sink}} \times \texttt{mem\_weight\_sink}
\]
where $n_{\text{sink}}$ is the number of sink particles in the grid,
determined by AMR tree descent from sink positions.
\end{quote}
...
%======================================================================
\section{\texttt{\&COSMO\_PARAMS} -- Cosmological Parameters}
\label{sec:cosmo_params}
%======================================================================

Optional block for overriding cosmological parameters read from the
IC file header.  Contains the four fundamental cosmological parameters:
\texttt{omega\_b}, \texttt{omega\_m}, \texttt{omega\_l}, and
\texttt{h0}.  These override the values stored in the GRAFIC2 IC
header.

Dark energy equation-of-state parameters ($w_0$, $w_a$, etc.)\ have
been moved to \texttt{\&CPL\_PARAMS}
(Section~\ref{sec:cpl_params}).  Neutrino parameters have been moved
to \texttt{\&NEUTRINO\_PARAMS} (Section~\ref{sec:neutrino_params}).

% ------------------------------------------------------------------
\paramheader{omega\_b}{real(dp)}{0.0}
\label{param:cosmo_omega_b}

\begin{quote}
Baryon density parameter $\Omega_b$.  Overrides the value set by
\texttt{\&PHYSICS\_PARAMS} or the IC header.
\end{quote}

% ------------------------------------------------------------------
\paramheader{omega\_m}{real(dp)}{1.0}
\label{param:cosmo_omega_m}

\begin{quote}
Total matter density parameter $\Omega_m$.  Read from the IC header
by default; this namelist entry overrides it.
\end{quote}

% ------------------------------------------------------------------
\paramheader{omega\_l}{real(dp)}{0.0}
\label{param:cosmo_omega_l}

\begin{quote}
Dark energy density parameter $\Omega_{\mathrm{de}}$ (equivalent to
$\Omega_\Lambda$ for $\Lambda$CDM).  Read from the IC header by
default; this namelist entry overrides it.
\end{quote}

% ------------------------------------------------------------------
\paramheader{h0}{real(dp)}{1.0}
\label{param:cosmo_h0}

\begin{quote}
Hubble constant $H_0$ in km\,s$^{-1}$\,Mpc$^{-1}$.  Read from the
IC header by default; this namelist entry overrides it.
\end{quote}



%======================================================================
\section{\texttt{\&CPL\_PARAMS} -- CPL Dark Energy}
\label{sec:cpl_params}
%======================================================================

Parameters for the CPL (Chevallier--Polarski--Linder) dark energy
equation of state.  When this block is absent, default values are used
($w_0=-1$, $w_a=0$, $c_s^2=0$), which recovers the standard
$\Lambda$CDM model with \textbf{bit-identical} results.

% ------------------------------------------------------------------
\paramheader{w0}{real(dp)}{$-1$}
\label{param:w0}

\begin{quote}
Present-day dark energy equation-of-state parameter $w_0$ in the CPL
parametrization:
\[
  w(a) = w_0 + w_a\,(1 - a)
\]
...
Setting $w_0 = -1$ and $w_a = 0$ (the defaults) exactly recovers the
cosmological constant $\Lambda$ (i.e.\ $\Lambda$CDM), producing
\textbf{bit-identical} results to the code before the CPL extension.

The dark energy density evolves as
\[
  \rho_{\mathrm{de}}(a) = \rho_{\mathrm{de},0}\;
  a^{-3(1+w_0+w_a)}\;\exp\!\bigl[-3\,w_a\,(1-a)\bigr]
\]
This modifies the Friedmann equation, Hubble rate $H(a)$, linear
growth factor $D_1(a)$, and growth rate $f(a)$.

\apinote{$w_0$ and $w_a$ are \emph{not} stored in the GRAFIC2 IC
header (which has a fixed 44-byte layout).  They must be specified
in this namelist block.  MUSIC generates CPL transfer functions
internally but does not write $w_0$, $w_a$ to its GRAFIC2 output.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{wa}{real(dp)}{0}
\label{param:wa}

\begin{quote}
Time-variation parameter $w_a$ of the CPL dark energy equation of
state.  See \paramref{w0} for the full parametrization.  Typical
observational constraints give $w_a \in [-1, 1]$.
\end{quote}

% ------------------------------------------------------------------
\paramheader{cs2\_de}{real(dp)}{0}
\label{param:cs2_de}

\begin{quote}
Dark energy sound speed squared $c_s^2$ (in units of $c^2$).  Used
in conjunction with \paramref{de_perturb}\texttt{=.true.}\ (set in
\texttt{\&RUN\_PARAMS}) to include dark energy perturbations in the
FFT Poisson solver.  Set to~0 for the standard smooth dark energy
(cosmological constant) assumption.

\textbf{Two DE perturbation modes} are available depending on whether
\paramref{de_table} is provided:

\begin{enumerate}[nosep]
  \item \textbf{Table-based linear response} (\paramref{de_table} set):
    Works for any $c_s^2$ including $c_s^2 = 0$ (clustering DE).
    The correction factor is
    $1 + (\rho_{\mathrm{de}}/\rho_{\mathrm{cb}})\,R_{\mathrm{DE}}(k,a)$,
    where $R_{\mathrm{DE}}$ is read from a CAMB transfer function table.
    When $c_s^2 < 10^{-4}$, the MG solver on AMR levels also applies
    a uniform boost (since $R_{\mathrm{DE}}$ is $k$-independent).

  \item \textbf{Quasi-static $\kappa^2/\alpha$ method} (no \paramref{de_table}, $c_s^2 > 0$):
    The FFT solver applies a $k$-dependent correction:
    $\hat\phi(\mathbf{k}) \leftarrow \hat\phi(\mathbf{k}) \times
    (k^2 + \kappa^2)/(k^2 + \kappa^2 + \alpha)$,
    where $\kappa^2$ and $\alpha$ are computed from $c_s^2$, $w(a)$, and
    the background DE density.
\end{enumerate}

Typical values: $c_s^2 = 1$ (speed-of-light sound speed, most common
in literature) or $c_s^2 = 0$ for clustering dark energy models
(requires \paramref{de_table}).

\apinote{The quasi-static method is only effective in the FFT solver
path.  The table-based method with $c_s^2 \approx 0$ also modifies the
MG V-cycle on AMR levels.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{de\_table}{character(256)}{'' (empty)}
\label{param:de_table}

\begin{quote}
Path to a CAMB dark energy transfer function ratio table for the
table-based linear response method.  The table contains the effective
$R_{\mathrm{DE}}(k, a)$ on a $(N_k \times N_a)$ grid, in the same
ASCII format as \paramref{neutrino_table}.

Generate with:
\begin{verbatim}
python3 misc/generate_de_table.py --w0=-0.9 --wa=0.1 --cs2=0.0 \
    -o de_table.dat
\end{verbatim}
The script uses the \textbf{Weyl potential method}: two CAMB runs
(target $c_s^2$ vs.\ reference $c_s^2=1$) extract the gauge-invariant
Weyl potential ratio, yielding an effective $R_{\mathrm{DE}}$ in the
Newtonian gauge.  This avoids the synchronous-gauge $\delta_{\rm DE}$
which can overestimate the correction by a factor of ${\sim}26$.
Default grid: 200~$k$-points $\times$ 50~$a$-points.

For $c_s^2 = 0$ (clustering DE), $R_{\mathrm{DE}}$ is independent of
$k$ at each $a$ --- this is a useful verification check.

\textbf{Requires} \paramref{de_perturb}\texttt{=.true.}\ (set in
\texttt{\&RUN\_PARAMS}).
When this parameter is empty, the code falls back to the quasi-static
$\kappa^2/\alpha$ method (which requires \paramref{cs2_de}~$> 0$).
\end{quote}


%======================================================================
\section{\texttt{\&NEUTRINO\_PARAMS} -- Massive Neutrino}
\label{sec:neutrino_params}
%======================================================================

Parameters for the massive neutrino linear response correction in the
Poisson solver.  Requires \paramref{use_neutrino}\texttt{=.true.}\ in
\texttt{\&RUN\_PARAMS}.

% ------------------------------------------------------------------
\paramheader{omega\_nu}{real(dp)}{0}
\label{param:omega_nu}

\begin{quote}
Massive neutrino density parameter $\Omega_\nu$.  The total matter
density $\Omega_m$ is interpreted as CDM~+~baryons~+~neutrinos, so the
CDM~+~baryon component is $\Omega_{\mathrm{cb}} = \Omega_m -
\Omega_\nu$.

For the standard minimal-mass normal hierarchy ($\Sigma m_\nu =
0.06\,$eV):
\[
  \Omega_\nu = \frac{\Sigma m_\nu}{93.14\,h^2\;\text{eV}}
  \approx 0.00140 \quad (h = 0.6774)
\]

Used by the neutrino linear response when
\paramref{use_neutrino}\texttt{=.true.}\ (set in
\texttt{\&RUN\_PARAMS}).

\apinote{When $\Omega_\nu = 0$, the code produces bit-identical results
regardless of \paramref{use_neutrino}.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{neutrino\_table}{character(256)}{'' (empty)}
\label{param:neutrino_table}

\begin{quote}
Path to the CAMB transfer function ratio table for neutrino linear
response.  The table contains $R_\nu(k, a) = T_\nu / T_{\mathrm{cb}}$
on a $(N_k \times N_a)$ grid.

\textbf{File format} (ASCII, space-separated):
\begin{verbatim}
Nk  Na
k_1  k_2  ...  k_Nk          ! wavenumbers [h/Mpc]
a_1  a_2  ...  a_Na          ! scale factors
R(k_1,a_1) R(k_2,a_1) ...   ! ratio values, row-major
R(k_1,a_2) R(k_2,a_2) ...
...
\end{verbatim}
...
Generate with:
\begin{verbatim}
python3 misc/generate_neutrino_table.py
\end{verbatim}
This script uses CAMB to compute the transfer functions for a given
neutrino mass sum (default $0.06\,$eV) and outputs a table with
200~$k$-points (log-spaced, $10^{-4}$--$10\;h/$Mpc) and 50~$a$-points
(log-spaced, $a_{\min}$--$1$).

\textbf{Requires} \paramref{use_neutrino}\texttt{=.true.}\ and
\paramref{omega_nu}~$> 0$.
\end{quote}


%======================================================================
\section{\texttt{\&SIDM\_PARAMS} -- Self-Interacting Dark Matter}
\label{sec:sidm_params}
%======================================================================

Parameters for self-interacting dark matter (SIDM) Monte Carlo
scattering.  Supports velocity-dependent cross-sections, anisotropic
and inelastic scattering.
Requires \paramref{sidm}\texttt{=.true.}\ and
\paramref{pic}\texttt{=.true.}\ in \texttt{\&RUN\_PARAMS}.

% ------------------------------------------------------------------
\paramheader{sidm\_cross\_section}{real(dp)}{1.0}
\label{param:sidm_cross_section}

\begin{quote}
DM self-interaction cross-section per unit mass $\sigma_0/m$ in
cm$^2$\,g$^{-1}$.  For velocity-dependent models (\texttt{sidm\_type}),
this is the reference value $\sigma_0$.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_type}{character(16)}{'constant'}
\label{param:sidm_type}

\begin{quote}
Cross-section velocity dependence:
\texttt{`constant'}: $\sigma/m = \sigma_0$;
\texttt{`yukawa'}: $\sigma/m = \sigma_0/(1+(v/v_0)^2)^2$;
\texttt{`power\_law'}: $\sigma/m = \sigma_0(v/v_0)^n$, capped at $10^4\sigma_0$.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_v0}{real(dp)}{100.0}
\label{param:sidm_v0}

\begin{quote}
Reference velocity $v_0$ in km/s for velocity-dependent models.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_power}{real(dp)}{$-4$}
\label{param:sidm_power}

\begin{quote}
Power-law index $n$ (only for \texttt{sidm\_type='power\_law'}).
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_npart\_min}{integer}{2}
\label{param:sidm_npart_min}

\begin{quote}
Minimum number of DM particles per cell required for scattering.
The default of~2 is the physical minimum.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_courant}{real(dp)}{0.1}
\label{param:sidm_courant}

\begin{quote}
Maximum allowed scattering probability per timestep.
The timestep is constrained by
$\Delta t_{\mathrm{new}} \le C_{\mathrm{SIDM}}/P_{\max} \times \Delta t_{\mathrm{old}}$.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_angular}{character(16)}{'isotropic'}
\label{param:sidm_angular}

\begin{quote}
Angular distribution of scattered velocity:
\texttt{`isotropic'} (uniform on sphere) or
\texttt{`rutherford'} (forward-peaked, $d\sigma/d\Omega \propto 1/(1-\cos\theta+2\epsilon)^2$).
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_epsilon}{real(dp)}{0.01}
\label{param:sidm_epsilon}

\begin{quote}
Regularisation parameter $\epsilon$ for Rutherford-like scattering.
Small values give stronger forward-peaking; $\epsilon\to\infty$ recovers isotropic.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_inelastic}{logical}{.false.}
\label{param:sidm_inelastic}

\begin{quote}
Enable inelastic scattering (iSIDM).  DM particles have ground (tp=0)
and excited (tp=$-1$) states with mass splitting $\delta$.
Channels: $g+g\to e+e$ (endothermic, threshold $\frac{1}{2}\mu v^2 > 2\delta c^2$),
$e+e\to g+g$ (exothermic), $g+e$ (elastic).
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_delta}{real(dp)}{0.0}
\label{param:sidm_delta}

\begin{quote}
Mass splitting $\delta$ in keV for inelastic scattering.
Required when \texttt{sidm\_inelastic=.true.}.
\end{quote}

% ------------------------------------------------------------------
\paramheader{sidm\_frac\_excited}{real(dp)}{0.0}
\label{param:sidm_frac_excited}

\begin{quote}
Initial fraction of DM particles in the excited state.
Applied once at startup (restart-safe: skipped if excited particles already exist).
\end{quote}

\paragraph{Diagnostic output.}
When scattering events occur, the code prints per-level statistics
including scattered count, pairs count, $P_{\max}$, conservation diagnostics
($\Delta\mathbf{p}$, $\Delta E_k$), and iSIDM up/down-scatter counts.
For elastic scattering, conservation is at machine precision
($\sim 10^{-15}$ relative).
For inelastic events, $\Delta E_k$ reflects the intentional $\pm 2\delta c^2$ transfer.


%======================================================================
\section{\texttt{\&MOND\_PARAMS} -- Modified Newtonian Dynamics}
\label{sec:mond_params}
%======================================================================

Parameters for MOND (Modified Newtonian Dynamics).  Three modes
are available: algebraic QUMOND (Phase~0), full QUMOND with phantom
density (Phase~1), and AQUAL iterative solver (Phase~2).
An external field effect (EFE) is supported in all modes.
Requires \paramref{use_mond}\texttt{=.true.}\ and
\paramref{poisson}\texttt{=.true.}\ in \texttt{\&RUN\_PARAMS}.

% ------------------------------------------------------------------
\paramheader{a0\_mond}{real(dp)}{$1.2\times10^{-8}$}
\label{param:a0_mond}

\begin{quote}
MOND critical acceleration $a_0$ in cm\,s$^{-2}$.  Converted to code
units internally via $a_0^{\rm code} = a_0 / (L_{\rm box}/t_{\rm code}^2)$.
Must be $> 0$.
\end{quote}

% ------------------------------------------------------------------
\paramheader{mond\_mu\_type}{integer}{1}
\label{param:mond_mu_type}

\begin{quote}
Interpolation function family:
\begin{description}[style=nextline,leftmargin=2cm,font=\ttfamily]
  \item[1] \textbf{Simple}: $\mu(x) = x/(1+x)$, \;
    $\nu(y) = \tfrac{1}{2} + \tfrac{1}{2}\sqrt{1 + 4/y}$.
  \item[2] \textbf{Standard}: $\mu(x) = x/\sqrt{1+x^2}$, \;
    $\nu(y) = \sqrt{\tfrac{1}{2} + \tfrac{1}{2}\sqrt{1 + 4/y^2}}$.
\end{description}
\end{quote}

% ------------------------------------------------------------------
\paramheader{mond\_type}{integer}{0}
\label{param:mond_type}

\begin{quote}
MOND implementation mode:
\begin{description}[style=nextline,leftmargin=2cm,font=\ttfamily]
  \item[0] \textbf{Algebraic QUMOND (Phase~0)}: cell-by-cell
    multiplication $\mathbf{f}^{\rm MOND} = \nu\,\mathbf{f}$.
    Fast but not curl-free for asymmetric mass distributions.
  \item[1] \textbf{Full QUMOND (Phase~1)}: compute phantom density
    $\rho_{\rm ph} = -\nabla\cdot[(\nu-1)\mathbf{f}]/(4\pi G)$
    and re-solve Poisson.  Guarantees curl-free MOND field.
    Cost: $\sim 2\times$ baseline Poisson.
  \item[2] \textbf{AQUAL iterative (Phase~2)}: self-consistent
    nonlinear Poisson solver via fixed-point iteration with
    $\mu$-function.  Most physically fundamental (derived from action).
    Cost: $(N_{\rm iter}+1)\times$ baseline Poisson.
    Controlled by \paramref{n_iter_mond} and \paramref{mond_eps}.
\end{description}
\end{quote}

% ------------------------------------------------------------------
\paramheader{n\_iter\_mond}{integer}{10}
\label{param:n_iter_mond}

\begin{quote}
Maximum number of AQUAL fixed-point iterations per level
(\texttt{mond\_type=2} only).  The iteration exits early if convergence
is reached (see \paramref{mond_eps}).  Must be $\geq 1$.

\apinote{Typical convergence requires 3--10 iterations depending on
the MOND regime.  The iteration count is printed at each step.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{mond\_eps}{real(dp)}{$10^{-6}$}
\label{param:mond_eps}

\begin{quote}
Convergence threshold for the AQUAL iteration
(\texttt{mond\_type=2} only).  The iteration converges when
$\max|\mathbf{f}^{(n+1)} - \mathbf{f}^{(n)}| /
\max|\mathbf{f}^{(n+1)}| < \epsilon$.
Must be $> 0$.

\apinote{A warning is printed if the iteration does not converge
within \paramref{n_iter_mond} steps.}
\end{quote}

% ------------------------------------------------------------------
\paramheader{g\_ext\_mond}{real(dp)(1:3)}{0, 0, 0}
\label{param:g_ext_mond}

\begin{quote}
External gravitational field $\mathbf{g}_{\rm ext}$ in cm\,s$^{-2}$
for the external field effect (EFE).  Applies to all MOND modes.
When non-zero, the interpolation function argument becomes
$|\mathbf{f} + \mathbf{g}_{\rm ext}|/a_0$ instead of
$|\mathbf{f}|/a_0$.

\begin{itemize}
\item Phase~0: $f_d^{\rm MOND} = \nu\,f_d + (\nu-1)\,g_{{\rm ext},d}$.
\item Phase~1: divergence uses $(\nu-1)(f_d + g_{{\rm ext},d})$.
\item Phase~2: divergence uses $(\mu-1)(f_d + g_{{\rm ext},d})$.
\end{itemize}

Set to $(0,0,0)$ for cosmological periodic boxes (no EFE).
Relevant for isolated galaxy or zoom-in simulations.
\end{quote}
...
&MOND_PARAMS
a0_mond      = 1.2d-8   ! MOND critical acceleration
mond_mu_type = 1         ! simple interpolation function
mond_type    = 2         ! AQUAL iterative
n_iter_mond  = 10        ! max iterations
mond_eps     = 1d-6      ! convergence threshold
g_ext_mond   = 0d0, 0d0, 1d-9  ! external field (optional)
/
\end{lstlisting}
...
! --- Optional: Self-Interacting Dark Matter ---
! &SIDM_PARAMS
! sidm_cross_section = 1.0   ! sigma/m in cm^2/g
! sidm_npart_min     = 2
! /
! (also set sidm = .true. in &RUN_PARAMS)

! --- Optional: MOND ---
! &MOND_PARAMS
! a0_mond      = 1.2d-8
! mond_mu_type = 1
! mond_type    = 1        ! 0=algebraic, 1=QUMOND, 2=AQUAL
! /
! (also set use_mond = .true. in &RUN_PARAMS)
\end{lstlisting}
...
\addlinespace
\texttt{use\_neutrino}
  & \texttt{omega\_nu}, \texttt{neutrino\_table}
  & All three must be set; \texttt{omega\_nu} and \texttt{neutrino\_table} are in \texttt{\&NEUTRINO\_PARAMS} \\
\addlinespace
\texttt{de\_perturb}
  & \texttt{cs2\_de}, \texttt{de\_table}, \texttt{w0}, \texttt{wa}
  & DE perturbation (in \texttt{\&RUN\_PARAMS}); DE parameters in \texttt{\&CPL\_PARAMS} \\
\addlinespace
\texttt{exchange\_method}
  & \texttt{ordering}
  & Auto-tune requires \texttt{ksection} ordering \\
\addlinespace
\texttt{memory\_balance}
  & \texttt{mem\_weight\_sink}
  & Sink cost weight for load balancing \\
\addlinespace
\texttt{sidm}
  & \texttt{sidm\_cross\_section}, \texttt{sidm\_npart\_min}
  & SIDM requires \texttt{pic=.true.}; parameters in \texttt{\&SIDM\_PARAMS} \\
\addlinespace
\texttt{use\_mond}
  & \texttt{a0\_mond}, \texttt{mond\_type}, \texttt{n\_iter\_mond}, \texttt{mond\_eps}, \texttt{g\_ext\_mond}
  & MOND requires \texttt{poisson=.true.}; parameters in \texttt{\&MOND\_PARAMS} \\
\end{longtable}
