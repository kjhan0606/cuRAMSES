...
{cuRAMSES~III: CPL Dark Energy Perturbations, Massive Neutrino
Linear Response, Self-Interacting Dark Matter, and QUMOND/AQUAL
...
code that enable cosmological hydrodynamic simulations with four
...
the need for neutrino particles; (iii)~self-interacting dark
...
(excited-state) scattering channels; and (iv)~Modified Newtonian
Dynamics in both the quasi-linear (QUMOND) and aquadratic-Lagrangian
(AQUAL) formulations, with three modes: an algebraic force correction,
a single-pass phantom density re-solve (QUMOND), and a self-consistent
iterative solver (AQUAL) that converges the nonlinear modified Poisson
equation via fixed-point iteration.  An external field effect (EFE) is
supported in all three modes for isolated galaxy simulations.  The dark energy and neutrino
modules are implemented within a unified Poisson solver framework in
which the corrections enter as multiplicative factors in Fourier
space, composing naturally with the existing multigrid and FFT solver
infrastructure.  The SIDM module uses cell-based random pairing with
Fisher--Yates shuffling, exact centre-of-mass-frame velocity updates
ensuring machine-precision momentum and energy conservation, and
deterministic OpenMP threading.  Each module is controlled by
dedicated namelist blocks and is verified to recover $\Lambda$CDM
results when disabled.  We describe the algorithms, unit conversions,
and numerical validation, providing a self-contained reference for
non-standard cosmological simulations with AMR codes.
\end{abstract}
...
In this paper we describe the implementation of four non-standard
...
\item \textbf{QUMOND} (Section~\ref{sec:mond}): Modified Newtonian
  Dynamics via the quasi-linear formulation, with two operational
  modes --- algebraic force correction and full phantom dark matter
  density re-solve --- providing a curl-free MOND gravitational
  field compatible with the AMR Poisson solver infrastructure.
\end{enumerate}
...
embedded.  Sections~\ref{sec:de}--\ref{sec:mond} describe each module
...
We have presented four non-standard cosmological modules for the
\curamses\ AMR code:
...
\item \textbf{QUMOND and AQUAL}: Modified Newtonian Dynamics via three
  modes: an algebraic force correction (Phase~0), a QUMOND phantom
  density re-solve (Phase~1), and an AQUAL iterative solver (Phase~2)
  that converges the nonlinear modified Poisson equation via
  fixed-point iteration (Section~\ref{sec:mond}).  An external field
  effect (EFE) is supported in all three modes.  All modes support
  simple and standard interpolation functions and are compatible
  with the cosmological Poisson solver.
\end{enumerate}

The dark energy and neutrino modules are designed within a unified
Poisson solver framework in which the corrections compose
...
SIDM, and MOND predictions for the halo mass function, galaxy
rotation curves, galaxy clustering, and weak lensing observables.
